...
const apiURL = "http://192.168.10.109:8090/";
//#region First Aid
...
function insertFirstAidAnswers(
  sqlTransaction: SQLTransaction,
  questionId: Number,
  answers: FirstAidAnswerModel[]
) {
    ...
    function (tx, res) {
      try {
        answers.forEach((data) => {
          ...
          tx.executeSql(
            "INSERT INTO FirstAidAnswer (answer, correctness, join_to_question) values ( ?, ?, ?)",
...
function insertFirstAidQuestions(
  dataArray: FirstAidQuestionModel[],
  callback: any
) {
        ...
        function (tx, res) {
          try {
            dataArray.forEach((data) => {
              ...
              tx.executeSql(
                "INSERT INTO FirstAidQuestion (question, sectionGroup, count) values ( ?, ?, ?)",
                [data.question, data.sectionGroup, data.count],
                (txObj, resultSet) => {
                  try {
                    console.log("insert question complete");
                    insertFirstAidAnswers(
                      txObj,
                      resultSet.insertId,
                      data.answers
                    );
...
export const getFirstAidQuestionsBySection = (
  section: string,
  callback: any
) => {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "SELECT fQ.id, fQ.question, fQ.sectionGroup, fQ.count, " +
    ...
    () => {
      console.log(
        "Transaction getFirstAidQuestionsBySection " + section + " done"
      );
    }
  );
};

export const deleteFirstAidQuestionsBySection = (
  section: string,
  callback: any
) => {
  db.transaction(
    (tx) => {
      tx.executeSql(
        "DELETE FROM FirstAidAnswer where FirstAidAnswer.id in (SELECT FirstAidQuestion.id from FirstAidQuestion inner join FirstAidAnswer on FirstAidQuestion.id = FirstAidAnswer.join_to_question where FirstAidQuestion.sectionGroup like ?)",
        [section],
        (tx, results) => {
          try {
            console.log(
              "From table FirstAidAnswer deleted all answers from question with section " +
                section
            );
            tx.executeSql(
              "DELETE FROM FirstAidQuestion where FirstAidQuestion.sectionGroup like ?",
              [section],
              (tx, results) => {
                console.log(
                  "From table FirstAidQuestion deleted every question with section " +
                    section
                );
                callback(true);
              },
...
export const getListOfInsertedQuestions = (callback: any) => {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "SELECT DISTINCT tQ.testnumber " + "FROM TestQuestion as tQ",
        [],
        (tx, results) => {
...
function insertDrivingTestAnswers(
  sqlTransaction: SQLTransaction,
  questionId: Number,
  answers: DrivingTestAnswerModel[]
) {
    ...
    function (tx, res) {
      try {
        answers.forEach((data) => {
          ...
          tx.executeSql(
            "INSERT INTO TestAnswer (answer, correctness, aChar, join_to_question) values ( ?, ?, ?, ?)",
...
function insertDrivingTestQuestions(
  dataArray: DrivingTestQuestionModel[],
  callback: any
) {
        ...
        function (tx, res) {
          try {
            dataArray.forEach((data) => {
              ...
              tx.executeSql(
                "INSERT INTO TestQuestion (question, imageURL, points, testgroup, testnumber) values ( ?, ?, ?, ?, ?)",
                [
                  data.question,
                  data.imageURL,
                  data.points,
...
export const downloadDrivingTestQuestions = (
  testnumber: number,
  callback: any
) => {
...
export const getDrivingTestQuestionsByNumber = (
  testnumber: number,
  callback: any
) => {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "SELECT tQ.id, tQ.question, tQ.imageURL, tQ.points, tQ.testgroup, tQ.testnumber, " +
    ...
    () => {
      console.log(
        "Transaction getDrivingTestQuestionsByNumber " + testnumber + " done"
      );
    }
  );
};

export const deleteDrivingTestQuestionsByNumber = (
  testnumber: string,
  callback: any
) => {
  db.transaction(
    (tx) => {
      tx.executeSql(
        "DELETE FROM TestAnswer where TestAnswer.id in (SELECT TestQuestion.id from TestQuestion inner join TestAnswer on TestQuestion.id = TestAnswer.join_to_question where TestQuestion.testnumber = ?)",
        [testnumber],
        (tx, results) => {
          try {
            console.log(
              "From table TestAnswer deleted all answers from question with testnumber " +
                testnumber
            );
            tx.executeSql(
              "DELETE FROM TestQuestion where TestQuestion.testnumber = ?",
              [testnumber],
              (tx, results) => {
                console.log(
                  "From table TestQuestion deleted every question with testnumber " +
                    testnumber
                );
                callback(true);
              },
    ...
    (error) => {
      console.log(
        "Transaction deleteDrivingTestQuestionsByNumber error",
        error
      );
      callback(false);
    },
...
function insertAssistantImgUrl(
  sqlTransaction: SQLTransaction,
  trafficSignId: Number,
  images: assistantImgUrlModel[]
) {
    ...
    function (tx, res) {
      try {
        images.forEach((data) => {
          ...
          tx.executeSql(
            "INSERT INTO AssistantTrafficSignImages (indX, url, join_to_trafficSign) values ( ?, ?, ?)",
        ...
        function (tx, res) {
          try {
            dataArray.forEach((data) => {
              ...
              tx.executeSql(
                "INSERT INTO TrafficSigns (title, body, imgUrl, section) values ( ?, ?, ?, ?)",
                ...
                (txObj, resultSet) => {
                  ...
                  if (data.assistantImages.length != 0) {
                    insertAssistantImgUrl(
                      txObj,
                      resultSet.insertId,
                      data.assistantImages
                    );
                  }
                },
                (TX, error) => {
...
export const getTrafficSignBySection = (section: number, callback: any) => {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "SELECT tS.id, tS.title, tS.body, tS.imgUrl, tS.section, " +
          "'[' || GROUP_CONCAT('{\"url\":' || '\"'  || aTSI.url || '\",' || ' \"index\":' || aTSI.indX  || '}' ) || ']' as assisImages " +
          ...
          try {
            for (let i = 0; i < results.rows.length; ++i) {
              ...
              if (results.rows.item(i).assisImages != null) {
                ...
                assitImagesStr = assitImagesStr.slice(
                  1,
                  assitImagesStr.length - 1
                );
    ...
    (error) => {
      console.log(
        "Transaction getTrafficSignBySection ",
        section,
        " error",
        error
      );
...
export const deleteTrafficSignsBySection = (section: string, callback: any) => {
  db.transaction(
    (tx) => {
      tx.executeSql(
        "DELETE FROM AssistantTrafficSignImages where AssistantTrafficSignImages.id in (SELECT TrafficSigns.id from TrafficSigns inner join AssistantTrafficSignImages on TrafficSigns.id = AssistantTrafficSignImages.join_to_trafficSign where TrafficSigns.section LIKE ?)",
        [section],
        (tx, results) => {
          tx.executeSql(
            "DELETE FROM TrafficSigns where TrafficSigns.section LIKE ?",
            [section],
            (tx, results) => {
              console.log(
                "From table TrafficSigns deleted every traffic sign with section " +
                  section
              );
              callback(true);
            },
      ...
      tx.executeSql(
        ...
        function (tx, res) {
          try {
            ...
            tx.executeSql(
              "INSERT INTO Notifications (testKind, testCathegory, testNumb, active, startDate, endDate) values ( ?, ?, ?, ?, ?, ?)",
              [
                testKind,
                testCathegory,
                testNumb,
              ...
              (txObj, resultSet) => {
                ...
                insertScheduledOneDayNotification(
                  txObj,
                  resultSet.insertId,
                  notifIDs,
                  scheduledDates
...
export function insertScheduledOneDayNotification(
  sqlTransaction: SQLTransaction,
  notificationJoinID: number,
  ...
  sqlTransaction.executeSql(
    "CREATE TABLE IF NOT EXISTS NotifSchedDay (notificationJoinID INTEGER, notificationID TEXT, scheduledDate INTEGER)",
    [],
    function (tx, res) {
      try {
        ...
        scheduledDates.forEach((dateSchel, index) => {
          console.log(
            "inserting insertScheduledOneDayNotification with date " +
              dateSchel,
            "and notificationID: ",
          ...
          tx.executeSql(
            "INSERT INTO NotifSchedDay (notificationJoinID, notificationID, scheduledDate) values ( ?, ?, ?)",
            [notificationJoinID, notificationIDs[index], dateSchel.getTime()],
            (txObj, resultSet) => {
              console.log(
                "insert insertScheduledOneDayNotification with date " +
                  dateSchel +
                  " completed "
              );
            },
...
export function deleteNotificationById(id: number, callback: any) {
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "DELETE FROM Notifications where Notifications.id = ?",
        [id],
        (tx, results) => {
...
function deleteOutDatedNotifications(
  ...
  sqlTransaction.executeSql(
    "DELETE FROM NotifSchedDay WHERE scheduledDate < ?",
    [currentDate.getTime()],
    function (tx, res) {
      tx.executeSql(
        "SELECT nt.id," +
        ...
        function (tx, res) {
          try {
            ...
            for (let i = 0; i < res.rows.length; ++i) {
                  if(res.rows.item(i).scheduledDates == null){
                        notifToDelete.push(res.rows.item(i).id);
                        argString = argString == "" ? " ? " : " ?, ";  
                  }
            }
            if(notifToDelete.length == 0){
                  ...
                  tx.executeSql(
                        "DELETE FROM Notifications WHERE id IN ("+argString+")",
                       [...notifToDelete],
                        function (tx, res) {
                              callback();
                        },
        ...
        function (tx, res) {
          try {
            tx.executeSql(
              ...
              function (tx, res) {
                try {
                  deleteOutDatedNotifications(new Date(Date.now()), tx, () => {
                    tx.executeSql(
                      "SELECT nt.id, nt.testKind, nt.testCathegory, nt.testNumb, nt.active, nt.startDate, nt.endDate, " +
                        "'[' || GROUP_CONCAT('{\"notificationID\":' || '\"'  || nsd.notificationID || '\",' || ' \"scheduledDate\":' || nsd.scheduledDate  || '}' ) || ']' as scheduledDates " +
                        "FROM Notifications AS nt LEFT JOIN NotifSchedDay AS nsd ON nt.id = nsd.notificationJoinID GROUP BY nt.id",
                      [],
                      (tx, results) => {
                        try {
                          for (let i = 0; i < results.rows.length; ++i) {
                            ...
                            var notif: NotificationModel = {
                              ...
                              scheduledDates:
                                new Array<NotificationScheduleDayModel>(),
                            };

                            if (results.rows.item(i).scheduledDates != null) {

                              var schedDatesStr: string =
                                results.rows.item(i).scheduledDates;
                              schedDatesStr = schedDatesStr.slice(
                                1,
                                schedDatesStr.length - 1
                              );
                              ...
                              arrayOfStr.forEach((data) => {
                                ...
                                var schedDateDay: NotificationScheduleDayModel =
                                  JSON.parse(data);
                            ...
                            notif.testCathegory =
                              results.rows.item(i).testCathegory;
...
export const updateStatusNotification = (
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        "UPDATE Notifications SET active = ? WHERE Notifications.id = ? AND Notifications.notifID = ?",
        [status, id, notifID],
        (tx, results) => {
          console.log(
